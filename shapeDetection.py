...
def getContours(img, imgContour):
    ...
    for cnt in contours:
        ...
        if area > 500:
            ...
            # cv2.putText(imgContour, "Area: "+str(int(area)), (x_+w_+20, y_+45), cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 0), 2)
...
while True:
    ...
    imgCan = cv2.Canny(imgGray, threshold1, threshold2)
    kernel = np.ones((5, 5))
    imgDil = cv2.dilate(imgCan, kernel, iterations=1)
    
    getContours(imgDil, imgContour)
    
    imgStack = stackImages(0.8, ([img, imgBlur, imgCan], [imgDil, imgContour, imgContour]))
